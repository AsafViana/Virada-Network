import { Box, Center, HStack, Pressable, ScrollView, Slider, Text, VStack } from 'native-base'
	...
	if (!fontsLoaded) {
		...
		return (
			<ScrollView w="100%" h="100%" bg={color.preto} overScrollMode="never">
				<Header />
				<Center bgColor={color.cinza} w={'100%'} h={'230'}>
					<HStack space={5} bg={color.branco} h={'70'} w={'350'} borderRadius={'3xl'} justifyContent={'center'} alignItems={'center'} >
						<Pressable onPress={() => setPlay(!Play)}>
							<Ionicons name={Play ? 'pause-outline' : 'play-outline'} color={color.preto} size={35} />
						</Pressable>
						<Slider defaultValue={0} colorScheme="blue" w={'200'}>
							<Slider.Track>
								<Slider.FilledTrack />
							</Slider.Track>
							<Slider.Thumb />
						</Slider>
						<Text color={color.preto} fontSize={15}>00:00</Text>
					</HStack>
				</Center>
